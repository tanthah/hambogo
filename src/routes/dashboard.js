...
router.get("/products", authenticateEmployee(), async (req, res) => {
  ...
  res.render(`${config.views}/dashboard/products.pug`, {
    products: products,
    msg: msg,
  });
});
...
router.get(
  ...
  async (req, res) => {
    ...
    res.render(`${config.views}/dashboard/editProduct.pug`, {
...
router.post(
  ...
  async (req, res) => {
    ...
    let large = {
      product_id: id,
      price: req.body.price_LARGE,
      stock: req.body.stock_LARGE,
      size: "LARGE",
    };
    let medium = {
      product_id: id,
      price: req.body.price_MEDIUM,
      stock: req.body.stock_MEDIUM,
      size: "MEDIUM",
    };
    let small = {
      product_id: id,
      price: req.body.price_SMALL,
      stock: req.body.stock_SMALL,
      size: "SMALL",
    };
    ...
    try {
      req.session.msg = await Products.updateAllDetails(
        product,
        [large, medium, small],
        id
      );
...
router.get("/accounts", authenticateAdmin(), async (req, res) => {
  ...
  res.render(`${config.views}/dashboard/accounts.pug`, {
    users: users,
    msg: msg,
  });
});
...
router.get(
  ...
  async (req, res) => {
    ...
    res.render(`${config.views}/dashboard/editAccount.pug`, {
      user: user,
    });
  }
);
...
function authenticateEmployee() {
  return (req, res, next) => {
    if (req.isAuthenticated() && res.locals.isAdmin) {
      return next(); // ✅ Admin có quyền employee
    }
    res.redirect("/login");
  };
}
